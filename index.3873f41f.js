 ...
 */function(t){return"undefined"!=typeof ArrayBuffer&&ArrayBuffer.isView?ArrayBuffer.isView(t):t&&t.buffer&&g(t.buffer)},isString:y,isNumber:b,isBoolean:t=>!0===t||!1===t,isObject:w,isPlainObject:E,isUndefined:d,isDate:A,isFile:v,isBlob:O,isRegExp:N,isFunction:m,isStream:t=>w(t)&&m(t.pipe),isURLSearchParams:B,isTypedArray:x,isFileList:R,forEach:S,merge:/**
...
return!D.isUndefined(e[t])});let n=r.metaTokens,i=r.visitor||l,o=r.dots,s=r.indexes,a=r.Blob||"undefined"!=typeof Blob&&Blob,u=a&&D.isSpecCompliantForm(e);if(!D.isFunction(i))throw TypeError("visitor must be a function");function f(t){if(null===t)return"";if(D.isDate(t))return t.toISOString();if(!u&&D.isBlob(t))throw new $("Blob is not supported. Use a Buffer instead.");return D.isArrayBuffer(t)||D.isTypedArray(t)?u&&"function"==typeof Blob?new Blob([t]):X.from(t):t}/**
   * Default visitor.
   *
   * @param {*} value
...
let n=t5.from("getAllResponseHeaders"in u&&u.getAllResponseHeaders()),i=s&&"text"!==s&&"json"!==s?u.response:u.responseText,o={data:i,status:u.status,statusText:u.statusText,headers:n,config:t,request:u};!function(t,e,r){let n=r.config.validateStatus;!r.status||!n||n(r.status)?t(r):e(new $("Request failed with status code "+r.status,[$.ERR_BAD_REQUEST,$.ERR_BAD_RESPONSE][Math.floor(r.status/100)-4],r.config,r.request,r))}(function(t){e(t),a()},function(t){r(t),a()},o),// Clean up request
...
u.send(i||null)})};const eo={http:null,xhr:ei};D.forEach(eo,(t,e)=>{if(t){try{Object.defineProperty(t,"name",{value:e})}catch(t){// eslint-disable-next-line no-empty
...
const{Axios:eE,AxiosError:eA,CanceledError:ev,isCancel:eO,CancelToken:eR,VERSION:eB,all:eS,Cancel:eT,isAxiosError:eU,spread:e_,toFormData:ex,AxiosHeaders:eC,HttpStatusCode:eI,formToJSON:eN,getAdapter:eL,mergeConfig:ej}=ew,eP=document.querySelector(".header"),eF=document.createElement("ul");eF.classList.add("movies-list");//nowe
const ek={};async function eD(){try{eP.after(eF);let t=await ew.get("https://api.themoviedb.org/3/trending/movie/day",{params:{language:"en-US",api_key:"c90cdec037818042646f6ab3cec9ea66"},headers:{accept:"application/json"}});t.data.results.forEach(t=>{//nowe
let e=t.genre_ids.map(t=>ek[t]);eF.insertAdjacentHTML("beforeend",`<li class='movie-box'>
        ...
        </li>`)})}catch{}}async function e$(t){try{let e=await ew.get("https://api.themoviedb.org/3/search/movie",{params:{query:`${t}`,api_key:"c90cdec037818042646f6ab3cec9ea66"},headers:{accept:"application/json"}});e.data.results.forEach(t=>{//nowe
let e=t.genre_ids.map(t=>ek[t]);eF.insertAdjacentHTML("beforeend",`<li class='movie-box'>
        <a class='movie-box__link'>
        <button class='movie-box__trailer-button' type='button' id=${t.id}>Trailer</button>
        <img class='movie-box__poster' id=${t.id} src='https://www.themoviedb.org/t/p/w500${t.poster_path}' />
        ...
        <h2 class='movie-box__title'>${t.title}</h2>
        <p class='movie-box__info'>${e.join(", ")} | ${t.release_date.slice(0,4)}</p>
        </li>`)})}catch{}}async function eM(t){try{let e=document.createElement("div");eF.after(e);let r=await ew.get(`https://api.themoviedb.org/3/movie/${t}`,{params:{language:"en-US",api_key:"c90cdec037818042646f6ab3cec9ea66"},headers:{accept:"application/json"}}),n=[];r.data.genres.forEach(t=>{let e=t.name;n.push(e)}),e.insertAdjacentHTML("afterbegin",`<img src='https://www.themoviedb.org/t/p/w500${r.data.poster_path}'/>
      <h2>${r.data.title}</h2>
      ...
      <p>Genre ${[...n]}</p>
    <p>ABOUT: ${r.data.overview}</p>`)}catch{}}async function eq(){try{//nowe
let t=await ew.get("https://api.themoviedb.org/3/genre/movie/list",{params:{language:"en",api_key:"c90cdec037818042646f6ab3cec9ea66"},headers:{accept:"application/json"}}),e=t.data.genres;e.forEach(t=>{ek[t.id]=t.name});// console.log(genres.data);
}catch{}}async function ez(t){try{let e=await ew.get(`https://api.themoviedb.org/3/movie/${t}/videos`,{params:{language:"en-US",api_key:"c90cdec037818042646f6ab3cec9ea66"},headers:{accept:"application/json"}}),r=e.data.results;for(let t of r)if("Trailer"===t.type){console.log(`https://youtu.be/${t.key}`);break}}catch{}}const eH=document.querySelector(".header__form"),eJ=document.querySelector("#input");window.addEventListener("load",()=>{eq(),eD()}),eH.addEventListener("submit",t=>{t.preventDefault(),eF.replaceChildren(),e$(eJ.value)}),eF.addEventListener("click",t=>{if("IMG"!==t.target.nodeName)return;let e=t.target.getAttribute("id");eM(e)}),eF.addEventListener("click",t=>{if("BUTTON"!==t.target.nodeName)return;let e=t.target.getAttribute("id");ez(e)});//# sourceMappingURL=index.3873f41f.js.map

//# sourceMappingURL=index.3873f41f.js.map
