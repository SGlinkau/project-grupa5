...
t=tI(t),!r&&e?"["+t+"]":t)}).join(r?".":""):e}let tL=D.toFlatObject(D,{},null,function(t){return/^is[A-Z]/.test(t)});var tj=/**
...
let ek={};async function eD(){try{eP.after(eF);let t=await ew.get("https://api.themoviedb.org/3/trending/movie/day",{params:{language:"en-US",api_key:"c90cdec037818042646f6ab3cec9ea66"},headers:{accept:"application/json"}});t.data.results.forEach(t=>{//nowe
...
}catch{}}async function ez(t){try{let e=await ew.get(`https://api.themoviedb.org/3/movie/${t}/videos`,{params:{language:"en-US",api_key:"c90cdec037818042646f6ab3cec9ea66"},headers:{accept:"application/json"}}),r=e.data.results;for(let t of r)if("Trailer"===t.type){console.log(`https://youtu.be/${t.key}`);break}}catch{}}let eH=document.querySelector(".header__form"),eJ=document.querySelector("#input");window.addEventListener("load",()=>{eq(),eD()}),eH.addEventListener("submit",t=>{t.preventDefault(),eF.replaceChildren(),e$(eJ.value)}),eF.addEventListener("click",t=>{if("IMG"!==t.target.nodeName)return;let e=t.target.getAttribute("id");eM(e)}),eF.addEventListener("click",t=>{if("BUTTON"!==t.target.nodeName)return;let e=t.target.getAttribute("id");ez(e)})}();//# sourceMappingURL=index.9e54d2e4.js.map

//# sourceMappingURL=index.9e54d2e4.js.map
